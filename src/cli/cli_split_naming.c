...
static plainsight_error plainsight_cli_parse_template(const char *template_text,
...
static plainsight_error plainsight_cli_write_u32_padded(char *out,
                                        size_t out_cap,
                                        uint32_t value,
...
plainsight_error plainsight_cli_split_build_default_template(const char *cover_path, char *out, size_t out_cap) {
    ...
    // Parse after build so the extension policy remains enforced by one validator
    {
        ...
        if (plainsight_cli_parse_template(out, &ignored_prefix, &ignored_width, &ignored_extension) != PLAINSIGHT_OK) {
...
plainsight_error plainsight_cli_split_format_shard_filename(const char *template_text,
    ...
    if (plainsight_cli_parse_template(template_text, &prefix_len, &pad_width, &extension_text) != PLAINSIGHT_OK) {
    ...
    if (plainsight_cli_write_u32_padded(out + prefix_len,
                                out_cap - prefix_len,
                                shard_index,
                                pad_width,
...
plainsight_error plainsight_cli_split_preflight_outputs(const char *output_dir, const char *template_text, uint32_t shard_count) {
    ...
    for (shard_index = 0u; shard_index < shard_count; shard_index++) {
        ...
        if (plainsight_cli_split_format_shard_filename(template_text, shard_index, shard_name, sizeof(shard_name)) != PLAINSIGHT_OK) {
            return PLAINSIGHT_ERR_ARGS;
        }
        if (plainsight_cli_join_dir_and_name(output_dir, shard_name, shard_path, sizeof(shard_path)) != PLAINSIGHT_OK) {
            return PLAINSIGHT_ERR_TOO_LARGE;
        }
        {
            int exists_flag = 0;
