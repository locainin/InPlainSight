...
static int plainsight_cli_byte_is_safe_filename_char(uint8_t value) {
  if ((value >= (uint8_t)'a' && value <= (uint8_t)'z') || (value >= (uint8_t)'A' && value <= (uint8_t)'Z') ||
...
static plainsight_error plainsight_cli_sanitize_inner_name(const uint8_t *name_bytes, size_t name_len,
  ...
  for (index = 0u; index < name_len; index++) {
    ...
    if (plainsight_cli_byte_is_safe_filename_char(byte_value) != 0 && byte_value != (uint8_t)'/' &&
        byte_value != (uint8_t)'\\') {
...
static plainsight_error plainsight_cli_resolve_extract_output_path(const char *requested_output_path,
                                                                   const char *payload_name, char *out,
                                                                   size_t out_cap) {
...
plainsight_error plainsight_cli_run_extract(const plainsight_extract_options *options) {
  ...
  result_code = plainsight_container_parse_outer(g_cli_workspace.container, container_length, &outer_header,
  ...
  result_code =
      plainsight_crypto_decrypt(encryption_key, outer_header.nonce, ciphertext_bytes, ciphertext_length,
                                g_cli_workspace.inner, sizeof(g_cli_workspace.inner), &decrypted_length);
  if (result_code != PLAINSIGHT_OK) {
    result_code = PLAINSIGHT_ERR_AUTH;
    goto cleanup;
  }
  ...
  if (inner_view.compression == PLAINSIGHT_COMPRESSION_ZSTD) {
    result_code = plainsight_decompress_zstd(inner_view.payload, (size_t)inner_view.payload_len,
                                             g_cli_workspace.payload, sizeof(g_cli_workspace.payload),
  ...
  result_code = plainsight_cli_sanitize_inner_name(inner_view.name, (size_t)inner_view.name_len,
                                                   sanitized_payload_name, sizeof(sanitized_payload_name));
  if (result_code != PLAINSIGHT_OK) {
    goto cleanup;
  }

  result_code = plainsight_cli_resolve_extract_output_path(
      options->output_path, sanitized_payload_name, resolved_output_path, sizeof(resolved_output_path));
  if (result_code != PLAINSIGHT_OK) {
    goto cleanup;
  }
