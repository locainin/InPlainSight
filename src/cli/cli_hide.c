...
static void plainsight_cli_log_hide_size_limits(uint64_t payload_file_size, uint64_t max_payload_by_cover) {
    ...
    plainsight_cli_write_u64_stderr((uint64_t)PLAINSIGHT_MAX_SINGLE_PAYLOAD_BYTES);
    (void)fputs(" bytes\n", stderr);
    (void)fputs("  hint: payload exceeds this cover; use split mode or choose/prepare a larger lossless cover\n", stderr);
}

static void plainsight_cli_log_hide_preflight(const plainsight_info_report *report,
    ...
    (void)fputs("  payload bytes: ", stderr);
    plainsight_cli_write_u64_stderr(original_payload_bytes);
    (void)fputc('\n', stderr);
    (void)fputs("  payload bytes after compression: ", stderr);
    plainsight_cli_write_u64_stderr(effective_payload_bytes);
    (void)fputc('\n', stderr);
    ...
    (void)fputs("  max payload by selected cover: ", stderr);
    plainsight_cli_write_u64_stderr(report->capacity.max_payload_by_cover_bytes);
    (void)fputc('\n', stderr);
    (void)fputs("  required shards: ", stderr);
    plainsight_cli_write_u64_stderr(report->required_shards);
    (void)fputc('\n', stderr);
}
...
static plainsight_error plainsight_cli_choose_auto_compression(size_t payload_length,
    ...
    *compressed_length_out = 0u;
    ...
    for (level_index = 0u; level_index < (sizeof(zstd_auto_levels) / sizeof(zstd_auto_levels[0])); level_index++) {
        size_t candidate_length = 0u;
        plainsight_error result_code = plainsight_compress_zstd_level(g_cli_workspace.payload,
                                                              payload_length,
                                                              zstd_auto_levels[level_index],
...
plainsight_error plainsight_cli_run_hide(const plainsight_hide_options *options) {
    ...
    if (options->compression_mode != PLAINSIGHT_COMPRESSION_NONE || payload_size_result != PLAINSIGHT_OK) {
        ...
        if (options->compression_mode == PLAINSIGHT_COMPRESSION_ZSTD) {
            result_code = plainsight_compress_zstd(g_cli_workspace.payload,
                                           payload_length,
            ...
            result_code = plainsight_cli_choose_auto_compression(payload_length, &compressed_length);
            if (result_code == PLAINSIGHT_OK) {
                final_compression_mode = PLAINSIGHT_COMPRESSION_ZSTD;
                effective_payload_file_size = (uint64_t)compressed_length;
            } else {
    ...
    plainsight_cli_log_hide_preflight(&info_report, payload_file_size, effective_payload_file_size, final_compression_mode);

    // Cover-derived max payload reflects all container overhead in this run
    if (effective_payload_file_size > max_payload_by_cover) {
        plainsight_cli_log_hide_size_limits(effective_payload_file_size, max_payload_by_cover);
    ...
    if (result_code != PLAINSIGHT_OK) {
        ...
        (void)fputs(options->output_path, stderr);
        (void)fputc('\n', stderr);
        (void)fputs("  hint: output must be a writable, non-existing lossless image path (.png, .jxl, .bmp, .ppm)\n",
                    stderr);
    }
