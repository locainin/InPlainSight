...
static plainsight_error plainsight_cli_info_payload_metadata(const plainsight_info_options *options,
                                                             ...
                                                             int *payload_provided, uint64_t *payload_bytes) {
  plainsight_error result_code = PLAINSIGHT_ERR_INTERNAL;
  size_t copied_name_length = 0u;

  if (options == NULL || payload_name_length == NULL || payload_provided == NULL || payload_bytes == NULL) {
  ...
  *payload_bytes = 0u;
  ...
  result_code = plainsight_io_copy_basename(options->payload_path, g_cli_workspace.payload_name,
                                            sizeof(g_cli_workspace.payload_name), &copied_name_length);
...
plainsight_error plainsight_cli_run_info(const plainsight_info_options *options) {
  ...
  result_code =
      plainsight_cli_info_payload_metadata(options, &payload_name_length, &payload_provided, &payload_bytes);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  {
    plainsight_info_report_input report_input = {&g_cli_workspace.image, detected_format,
                                                 options->lsb_bits,      options->density_per_mille,
                                                 payload_name_length,    PLAINSIGHT_MIME_OCTET_STREAM_LEN,
                                                 payload_provided,       payload_bytes};
