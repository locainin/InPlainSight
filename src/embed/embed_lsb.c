...
typedef struct plainsight_lsb_rotation {
  uint64_t value;
  unsigned int shift_count;
} plainsight_lsb_rotation;

static uint64_t plainsight_lsb_rotate_left_u64(const plainsight_lsb_rotation *rotation) {
  uint64_t value = rotation->value;
  unsigned int normalized_shift = rotation->shift_count & 63u;
...
typedef struct plainsight_lsb_seed_domain {
  const uint8_t *seed_bytes;
  uint64_t domain_size;
  uint8_t domain_label;
} plainsight_lsb_seed_domain;

static uint64_t plainsight_lsb_mix_seed_u64(const plainsight_lsb_seed_domain *domain) {
  const uint8_t *seed_bytes = domain->seed_bytes;
  uint64_t domain_size = domain->domain_size;
  uint8_t domain_label = domain->domain_label;
  ...
  mixed_value = lane0;
  {
    plainsight_lsb_rotation rotation = {lane1, 17u};
    mixed_value ^= plainsight_lsb_rotate_left_u64(&rotation);
  }
  {
    plainsight_lsb_rotation rotation = {lane2, 31u};
    mixed_value ^= plainsight_lsb_rotate_left_u64(&rotation);
  }
  {
    plainsight_lsb_rotation rotation = {lane3, 47u};
    mixed_value ^= plainsight_lsb_rotate_left_u64(&rotation);
  }
...
static plainsight_error plainsight_lsb_compute_permutation(const uint8_t seed_bytes[32], uint64_t domain_size,
                                                           uint64_t *step_out, uint64_t *bias_out) {
  ...
  {
    plainsight_lsb_seed_domain seed_domain = {seed_bytes, domain_size, 0xA5u};
    permutation_step = plainsight_lsb_mix_seed_u64(&seed_domain);
  }
  ...
  {
    plainsight_lsb_seed_domain seed_domain = {seed_bytes, domain_size, 0x5Au};
    permutation_bias = plainsight_lsb_mix_seed_u64(&seed_domain);
  }
...
static uint64_t plainsight_lsb_advance_permutation(uint64_t current_index, uint64_t permutation_step,
                                                   uint64_t domain_size) {
...
typedef struct plainsight_lsb_cover_slot {
  const uint8_t *cover_bytes;
  size_t cover_length;
  uint64_t cover_index;
  uint8_t cover_channel_stride;
} plainsight_lsb_cover_slot;

static uint16_t plainsight_lsb_texture_score(const plainsight_lsb_cover_slot *slot) {
  const uint8_t *cover_bytes = slot->cover_bytes;
  size_t cover_length = slot->cover_length;
  uint64_t cover_index = slot->cover_index;
  uint8_t cover_channel_stride = slot->cover_channel_stride;
...
typedef struct plainsight_lsb_slot_check {
  plainsight_lsb_cover_slot slot;
  uint16_t minimum_texture_score;
} plainsight_lsb_slot_check;

static int plainsight_lsb_slot_is_usable(const plainsight_lsb_slot_check *check) {
  uint16_t slot_score = plainsight_lsb_texture_score(&check->slot);
  return slot_score >= check->minimum_texture_score;
}

typedef struct plainsight_lsb_phase_slot {
  unsigned int phase_index;
  int slot_is_high_texture;
} plainsight_lsb_phase_slot;

static int plainsight_lsb_slot_matches_phase(const plainsight_lsb_phase_slot *phase_slot) {
  if (phase_slot->phase_index == 0u) {
    return phase_slot->slot_is_high_texture != 0;
  }
  return phase_slot->slot_is_high_texture == 0;
}

typedef struct plainsight_lsb_length_bit_input {
  size_t payload_length;
  uint64_t bit_index;
} plainsight_lsb_length_bit_input;

static uint8_t plainsight_lsb_length_bit(const plainsight_lsb_length_bit_input *input) {
  size_t length_byte_index = (size_t)(input->bit_index / 8u);
  unsigned int bit_offset = (unsigned int)(input->bit_index % 8u);
  uint64_t payload_length_u64 = (uint64_t)input->payload_length;
...
typedef struct plainsight_lsb_payload_bit_write {
  uint64_t payload_bit_index;
  uint8_t bit_value;
} plainsight_lsb_payload_bit_write;

static void plainsight_lsb_write_payload_bit(uint8_t *payload_bytes,
                                             const plainsight_lsb_payload_bit_write *bit_write) {
  size_t payload_byte_index = (size_t)(bit_write->payload_bit_index / 8u);
  unsigned int payload_bit_offset = (unsigned int)(bit_write->payload_bit_index % 8u);
  uint8_t bit_mask = (uint8_t)(1u << payload_bit_offset);

  // Toggle only the addressed bit so neighboring bits remain unchanged
  if (bit_write->bit_value == 0u) {
...
plainsight_error plainsight_embed_lsb_payload(uint8_t *cover, size_t cover_len, uint8_t cover_channel_stride,
  ...
  result_code =
      plainsight_lsb_compute_permutation(seed, (uint64_t)cover_len, &permutation_step, &permutation_bias);
  ...
  for (phase_index = 0u; phase_index < 2u && embedded_bit_count < required_bit_count; phase_index++) {
    ...
    for (permutation_index = 0u;
      ...
      plainsight_lsb_slot_check slot_check = {{cover, cover_len, selected_cover_index, cover_channel_stride},
                                              minimum_texture_score};
      int slot_is_high_texture = plainsight_lsb_slot_is_usable(&slot_check);
      ...
      {
        plainsight_lsb_phase_slot phase_slot = {phase_index, slot_is_high_texture};
        if (!plainsight_lsb_slot_matches_phase(&phase_slot)) {
          continue;
        }
      }

      if (embedded_bit_count < PLAINSIGHT_LSB_PREFIX_BITS) {
        plainsight_lsb_length_bit_input length_input = {payload_len, embedded_bit_count};
        bit_to_embed = plainsight_lsb_length_bit(&length_input);
      } else {
...
plainsight_error plainsight_extract_lsb_payload(const uint8_t *cover, size_t cover_len,
                                                uint8_t cover_channel_stride, uint8_t *payload_out,
  ...
  for (phase_index = 0u; phase_index < 2u && observed_slot_count < PLAINSIGHT_LSB_PREFIX_BITS;
       phase_index++) {
    uint64_t cover_index = permutation_bias;
  ...
  for (phase_index = 0u; phase_index < 2u && observed_slot_count < required_bit_count; phase_index++) {
    ...
    for (permutation_index = 0u;
      ...
      if (observed_slot_count >= PLAINSIGHT_LSB_PREFIX_BITS) {
        plainsight_lsb_payload_bit_write bit_write = {observed_slot_count - PLAINSIGHT_LSB_PREFIX_BITS,
                                                      embedded_bit};
        plainsight_lsb_write_payload_bit(payload_out, &bit_write);
      }
