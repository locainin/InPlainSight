...
plainsight_error plainsight_info_build_report(const plainsight_info_report_input *input,
  ...
  const plainsight_image *image = NULL;
  plainsight_image_format format = PLAINSIGHT_IMAGE_FORMAT_UNKNOWN;
  uint8_t lsb_bits = 0u;
  uint16_t density_per_mille = 0u;
  size_t payload_name_len = 0u;
  size_t mime_len = 0u;
  int payload_provided = 0;
  uint64_t payload_bytes = 0u;

  if (input == NULL || input->image == NULL || report == NULL) {
    return PLAINSIGHT_ERR_ARGS;
  }
  image = input->image;
  format = input->format;
  lsb_bits = input->lsb_bits;
  density_per_mille = input->density_per_mille;
  payload_name_len = input->payload_name_len;
  mime_len = input->mime_len;
  payload_provided = input->payload_provided;
  ...
  report->max_payload_per_shard_bytes = plainsight_info_min_u64(
      report->capacity.max_payload_by_cover_bytes, (uint64_t)PLAINSIGHT_MAX_SHARD_PLAINTEXT_BYTES);
  ...
  if (payload_provided != 0) {
    ...
    if (report->max_payload_per_shard_bytes > 0u) {
      ...
      if (report->fits_single == 0) {
        plainsight_split_plan_input split_input = {payload_bytes, shard_capacity};
    ...
    if (report->fits_single == 0 && report->required_shards > 1u &&
...
plainsight_error plainsight_info_write_json_stdout(const plainsight_info_report *report,
                                                   const char *limiting_factor_text) {
