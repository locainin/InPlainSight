...
static const uint8_t PLAINSIGHT_SPLIT_MANIFEST_MAGIC[4] = {(uint8_t)'H', (uint8_t)'I', (uint8_t)'S',
                                                           (uint8_t)'M'};
...
typedef struct plainsight_split_manifest_size_input {
  uint32_t shard_count;
  uint8_t flags;
} plainsight_split_manifest_size_input;

static plainsight_error
plainsight_split_manifest_expected_len(const plainsight_split_manifest_size_input *input,
                                       size_t *expected_len_out) {
  size_t expected_len = 0u;
  uint32_t shard_count = 0u;
  uint8_t flags = 0u;

  if (input == NULL || expected_len_out == NULL) {
...
plainsight_error plainsight_split_manifest_pack(const plainsight_split_manifest_pack_input *input,
  ...
  size_t expected_len = 0u;
  uint8_t manifest_version = 0u;
  uint8_t flags = 0u;
  const uint8_t *set_id = NULL;
  uint64_t total_plaintext_len = 0u;
  uint8_t compression_mode = 0u;
  uint32_t chunk_plain_len = 0u;
  uint32_t shard_count = 0u;
  const uint32_t *per_shard_plain_len = NULL;
  const uint64_t *per_shard_cipher_len = NULL;
  ...
  flags = input->flags;
  set_id = input->set_id;
  total_plaintext_len = input->total_plaintext_len;
  compression_mode = input->compression_mode;
  chunk_plain_len = input->chunk_plain_len;
  shard_count = input->shard_count;
  per_shard_plain_len = input->per_shard_plain_len;
  per_shard_cipher_len = input->per_shard_cipher_len;

  if (set_id == NULL || per_shard_plain_len == NULL) {
    return PLAINSIGHT_ERR_ARGS;
  }
  ...
  {
    plainsight_split_manifest_size_input size_input = {shard_count, flags};
    if (plainsight_split_manifest_expected_len(&size_input, &expected_len) != PLAINSIGHT_OK) {
      return PLAINSIGHT_ERR_TOO_LARGE;
    }
  }
...
plainsight_error plainsight_split_manifest_parse(const uint8_t *in, size_t in_len,
  ...
  if (in[0] != PLAINSIGHT_SPLIT_MANIFEST_MAGIC[0] || in[1] != PLAINSIGHT_SPLIT_MANIFEST_MAGIC[1] ||
      in[2] != PLAINSIGHT_SPLIT_MANIFEST_MAGIC[2] || in[3] != PLAINSIGHT_SPLIT_MANIFEST_MAGIC[3]) {
    return PLAINSIGHT_ERR_BAD_FORMAT;
  }
  read_offset += 4u;
  ...
  {
    plainsight_split_manifest_size_input size_input = {manifest_view->shard_count, manifest_view->flags};
    if (plainsight_split_manifest_expected_len(&size_input, &expected_len) != PLAINSIGHT_OK) {
      return PLAINSIGHT_ERR_BAD_FORMAT;
    }
  }
...
plainsight_error plainsight_split_manifest_plain_len_at(const plainsight_split_manifest_view *manifest_view,
  ...
  *plain_len_out = plainsight_split_manifest_read_u32_le(manifest_view->per_shard_plain_len_bytes +
                                                         ((size_t)shard_index * 4u));
  return PLAINSIGHT_OK;
}
...
plainsight_error plainsight_split_manifest_cipher_len_at(const plainsight_split_manifest_view *manifest_view,
  ...
  *cipher_len_out = plainsight_split_manifest_read_u64_le(manifest_view->per_shard_cipher_len_bytes +
                                                          ((size_t)shard_index * 8u));
  return PLAINSIGHT_OK;
}
