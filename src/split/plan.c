...
static plainsight_error plainsight_split_plan_manifest_len(uint32_t shard_count, size_t *manifest_len_out) {
  ...
  manifest_len =
      (size_t)PLAINSIGHT_SPLIT_MANIFEST_BASE_BYTES + ((size_t)shard_count * 4u) + ((size_t)shard_count * 8u);
...
plainsight_error plainsight_split_plan_compute(const plainsight_split_plan_input *input,
  ...
  uint64_t per_shard_plain_capacity = 0u;

  if (input == NULL || plan_out == NULL) {
    return PLAINSIGHT_ERR_ARGS;
  }
  payload_bytes = input->payload_bytes;
  per_shard_plain_capacity = input->per_shard_plain_capacity;
