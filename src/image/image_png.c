...
typedef struct plainsight_png_geometry {
  uint32_t width;
  uint32_t height;
  uint64_t total_bytes;
} plainsight_png_geometry;

static plainsight_error plainsight_png_validate_geometry(const plainsight_png_geometry *geometry) {
  if (geometry == NULL) {
  ...
  if (geometry->width > PLAINSIGHT_MAX_IMAGE_DIMENSION || geometry->height > PLAINSIGHT_MAX_IMAGE_DIMENSION) {
    return PLAINSIGHT_ERR_TOO_LARGE;
  }

  // Total byte cap matches fixed in-memory pixel buffer
  if (geometry->total_bytes > PLAINSIGHT_MAX_IMAGE_BYTES) {
    return PLAINSIGHT_ERR_TOO_LARGE;
  }
...
plainsight_error plainsight_image_png_read(const char *path, plainsight_image *image) {
  ...
  decoded_rgb_bytes = (uint64_t)png_image_state.width * (uint64_t)png_image_state.height * 3u;
  if (decoded_rgb_bytes > (uint64_t)PNG_UINT_31_MAX) {
    png_image_free(&png_image_state);
    return PLAINSIGHT_ERR_TOO_LARGE;
  }
  png_image_bytes = (png_alloc_size_t)decoded_rgb_bytes;

  {
    plainsight_png_geometry geometry = {png_image_state.width, png_image_state.height,
                                        (uint64_t)png_image_bytes};
    result_code = plainsight_png_validate_geometry(&geometry);
  }
...
plainsight_error plainsight_image_png_write(const char *path, const plainsight_image *image) {
  ...
  {
    plainsight_png_geometry geometry = {image->width, image->height, expected_bytes};
    if (plainsight_png_validate_geometry(&geometry) != PLAINSIGHT_OK) {
      return PLAINSIGHT_ERR_TOO_LARGE;
    }
  }
