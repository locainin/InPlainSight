...
static int plainsight_ppm_is_whitespace(uint8_t character) {
  return character == (uint8_t)' ' || character == (uint8_t)'\t' || character == (uint8_t)'\n' ||
         character == (uint8_t)'\r' || character == (uint8_t)'\f' || character == (uint8_t)'\v';
}

static plainsight_error plainsight_ppm_next_token(const uint8_t *input_bytes, size_t input_length,
                                                  size_t *cursor_offset, char *token_out,
                                                  size_t token_capacity, size_t *token_length_out) {
  size_t token_length = 0u;
  size_t cursor = 0u;

  if (input_bytes == NULL || cursor_offset == NULL || token_out == NULL || token_capacity < 2u ||
      token_length_out == NULL) {
...
static plainsight_error plainsight_ppm_consume_raster_delimiter(const uint8_t *input_bytes,
                                                                size_t input_length, size_t *cursor_offset) {
  size_t cursor = 0u;
...
typedef struct plainsight_ppm_geometry {
  uint32_t image_width;
  uint32_t image_height;
  uint64_t rgb_bytes;
} plainsight_ppm_geometry;

static plainsight_error plainsight_ppm_validate_geometry(const plainsight_ppm_geometry *geometry) {
  if (geometry == NULL) {
    return PLAINSIGHT_ERR_ARGS;
  }
...
static plainsight_error plainsight_ppm_copy_bytes(uint8_t *destination, size_t destination_capacity,
                                                  size_t *destination_offset, const uint8_t *source,
...
static plainsight_error plainsight_ppm_append_ascii(uint8_t *destination, size_t destination_capacity,
                                                    size_t *destination_offset, const char *text_value) {
  ...
  return plainsight_ppm_copy_bytes(destination, destination_capacity, destination_offset,
                                   (const uint8_t *)text_value, text_length);
}

static plainsight_error plainsight_ppm_append_u32_decimal(uint8_t *destination, size_t destination_capacity,
  ...
  return plainsight_ppm_copy_bytes(destination, destination_capacity, destination_offset, forward_digits,
                                   reverse_length);
}
...
plainsight_error plainsight_image_ppm_read(const char *path, plainsight_image *image) {
  ...
  result_code = plainsight_ppm_next_token(g_plainsight_image_input_bytes, input_length, &cursor_offset,
                                          token_text, sizeof(token_text), &token_length);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  {
    plainsight_ppm_geometry geometry = {image_width, image_height, rgb_bytes};
    result_code = plainsight_ppm_validate_geometry(&geometry);
  }
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  result_code =
      plainsight_ppm_consume_raster_delimiter(g_plainsight_image_input_bytes, input_length, &cursor_offset);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
...
plainsight_error plainsight_image_ppm_write(const char *path, const plainsight_image *image) {
  ...
  result_code =
      plainsight_ppm_append_u32_decimal(header_text, sizeof(header_text), &header_length, image_width);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  result_code =
      plainsight_ppm_append_u32_decimal(header_text, sizeof(header_text), &header_length, image_height);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  result_code =
      plainsight_ppm_copy_bytes(g_plainsight_image_output_bytes, sizeof(g_plainsight_image_output_bytes),
                                &output_offset, header_text, header_length);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
  ...
  result_code =
      plainsight_ppm_copy_bytes(g_plainsight_image_output_bytes, sizeof(g_plainsight_image_output_bytes),
                                &output_offset, image->pixels, (size_t)rgb_bytes);
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
