...
typedef struct plainsight_bmp_geometry {
  uint32_t image_width;
  uint32_t image_height;
  uint64_t rgb_bytes;
} plainsight_bmp_geometry;

static plainsight_error plainsight_bmp_validate_geometry(const plainsight_bmp_geometry *geometry) {
  if (geometry == NULL) {
...
plainsight_error plainsight_image_bmp_read(const char *path, plainsight_image *image) {
  ...
  if (g_plainsight_image_input_bytes[0] != (uint8_t)'B' ||
      g_plainsight_image_input_bytes[1] != (uint8_t)'M') {
    return PLAINSIGHT_ERR_BAD_FORMAT;
  }
  ...
  {
    plainsight_bmp_geometry geometry = {image_width, image_height, rgb_bytes};
    result_code = plainsight_bmp_validate_geometry(&geometry);
  }
  if (result_code != PLAINSIGHT_OK) {
    return result_code;
  }
