...
int main(void) {
  ...
  plainsight_split_plan_input tiny_input = {1u, 1000u};
  result_code = plainsight_split_plan_compute(&tiny_input, &plan);
  ...
  plainsight_split_plan_input multi_input = {2000u, 1000u};
  result_code = plainsight_split_plan_compute(&multi_input, &plan);
  ...
  plainsight_split_plan_input tiny_capacity_input = {1u, 32u};
  result_code = plainsight_split_plan_compute(&tiny_capacity_input, &plan);
  if (!check_true(result_code == PLAINSIGHT_ERR_CAPACITY,
                  "expected capacity failure when shard too small for manifest")) {
    return 1;
  }
