...
// Report input keeps all planning knobs grouped and named at the call site
typedef struct plainsight_info_report_input {
  // Decoded cover image used for capacity math
  const plainsight_image *image;
  // Original cover format
  plainsight_image_format format;
  // LSB method bits, currently one bit
  uint8_t lsb_bits;
  // Usable-pixel density in per-mille
  uint16_t density_per_mille;
  // Payload name bytes included in container overhead
  size_t payload_name_len;
  // MIME text bytes included in container overhead
  size_t mime_len;
  // Whether payload_bytes is known for this preflight
  int payload_provided;
  // Optional payload size
  uint64_t payload_bytes;
} plainsight_info_report_input;
