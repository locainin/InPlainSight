...
plainsight_error plainsight_cli_read_exact_bytes(int file_descriptor, uint8_t *out, size_t to_read,
                                                 size_t *read_out);
...
plainsight_error plainsight_cli_open_temp_output_exclusive(const char *final_path, char *temp_path,
                                                           size_t temp_cap, int *file_descriptor_out);
...
// Reads the embedded container bytes from one stego image, then parses split outer v2
...
plainsight_error plainsight_cli_split_read_shard(const char *path, plainsight_embed_method method,
                                                 const uint8_t stego_subkey[32], uint8_t *container_out,
                                                 size_t container_cap, size_t *container_len_out,
