...
pub fn show_info_dialog(
    ...
    // Reuse the generic choice dialog so all modal styling stays consistent
    show_choice_dialog(parent_window, title_text, detail_text, &["OK"], 0, |_| {});
}

// Show a small modal dialog and report which button was clicked
...
{
    // Use a custom GTK window so CSS matches the rest of the application
    ...
    // The callback is consumed once to prevent duplicate action execution
    let callback = std::rc::Rc::new(std::cell::RefCell::new(Some(on_choice)));
    for (index, label_text) in button_labels.iter().enumerate() {
        // The default action gets primary styling while other actions stay secondary
        ...
        button.connect_clicked(move |_| {
            // Closing first makes the UI respond immediately before follow-up work runs
            dialog_clone.close();
