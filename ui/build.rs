...
fn main() {
    ...
    match build_result {
        ...
        Ok(status) => {
            panic!(
                "failed to build C CLI via make (exit status: {})\n\
        ...
        Err(error) => {
            panic!(
                "failed to run make for C CLI build: {}\n\
...
fn emit_rerun_if_changed(path_value: &Path) {
    ...
    let entries = match fs::read_dir(path_value) {
        Ok(read_directory) => read_directory,
        Err(_) => return,
    };

    for entry_result in entries {
        let entry_value = match entry_result {
